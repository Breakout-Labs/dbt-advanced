...
add_store_id as (
    ...
    renamed as (select *, id as order_id, created_at as ordered_at from add_store_id),
    ...
    normalize_order_status as (
        ...
        from renamed
        left join order_status on (lower(renamed.status) = order_status.order_status)
    ),

    final as (select * from normalize_order_status)
